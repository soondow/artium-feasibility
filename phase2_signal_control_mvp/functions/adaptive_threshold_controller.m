...
% data collection instead of becoming direct alerts.
    ...
    for k = 1:n
        ...
        % Direct alert is allowed only when the observation is strong and the
        % measurement context is reliable. This is the core low-quality alert
        % prohibition used throughout the Phase 2 tests.
        reliableForAlert = q(k) >= params.minQualityForAlert ...
            && coverage(k) >= params.minCoverageForAlert ...
            && U(k) <= params.maxUncertaintyForAlert;

        % Suspicious but weak evidence is still acted on, but as a monitoring
        % request. This keeps low-quality artifacts visible without counting
        % them as user-facing direct alerts.
        ...
        % The policy label is intentionally separated from the raw masks until
        % refractory and dwell rules are applied, because adjacent overlapping
        % windows should not create noisy action switching.
        ...
        % After dwell control, synchronize masks to the selected policy so the
        % logger and episode metrics describe the same user-facing action.
