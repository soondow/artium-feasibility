clear; clc; close all;
% Phase 3 evaluates whether MSPC can serve as an additional observation
% stream. It does not replace the Phase 2 quality-aware controller.
...
% Train the normal-operation model only on reliable non-risk windows. This
% keeps MSPC focused on process deviation rather than learning artifact or
% injected risk episodes as normal behavior.
...
function T = buildMspcEventMetrics(W, mspcRisk)
    ...
    for s = 1:numel(scenarios)
        ...
        for m = 1:numel(methods)
            % Every observation candidate is passed through the same Phase 2
            % observer-controller stack so MSPC is judged by monitoring burden,
            % not by raw anomaly score alone.
